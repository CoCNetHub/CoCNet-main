...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    reverse_blur_path = '/root/autodl-tmp/attributeandsubjectandspatial/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeandsubjectandspatial/' + "yuanben" + img_file[int(img_file.rfind('/')) + 1:]
...
class Predictor(object):
        ...
        for index in range(len(objects)):
            ...
            if (subject != ''):
                if (clsNameItem == subject):
                    ...
                    ROI = crop.filter(ImageFilter.GaussianBlur(radius=50))
    ...
    def allobjects(self, objects, img_file):
        ...
        for index in range(len(objects)):
            ...
            # Reverse blur
            reverse_blur_img = Image.open(img_file)
            ROI = crop.filter(ImageFilter.GaussianBlur(radius=80))
            reverse_blur_img.paste(ROI, (int(bbox[0]), int(bbox[1])))
            plt.imshow(reverse_blur_img)
            plt.show()
            # Blur
            blur_all_img = Image.open(img_file)
            blur_all_img = blur_all_img.filter(ImageFilter.GaussianBlur(radius=140))
            blur_all_img.paste(crop, (int(bbox[0]), int(bbox[1])))
        ...
        for file in files:
            ...
            for subFile in subFileDir:
                ...
                for i in range(len(data)):
                    ...
                    path = "/root/autodl-tmp/result/attributeandsubjectandspatial/" + file + "_attributeandsubjectandspatial_adjust_" + clip_model_list_item_str + '' + annotationFileName
                    fo = open(path, "a")
                    fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

                attribute_result_path = "/root/autodl-tmp/result/attributeandsubjectandspatial/"+"attributeandsubjectandspatial_adjust1_"+clip_model_list_item_str+".csv"
