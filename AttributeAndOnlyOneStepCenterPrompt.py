...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeandonlyonestepcenterprompt', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    mask_path = '/root/autodl-tmp/attributeandonlyonestepcenterprompt/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeandonlyonestepcenterprompt/' + "yuanben" + img_file[int(img_file.rfind('/')) + 1:]
        ...
        for index in range(len(objects)):
            ...
            if (subject != ''):
                if (clsNameItem == subject):
                    ...
                    crop_path = '/root/autodl-tmp/attributeandonlyonestepcenterprompt/' + str(index) + '.png'
                    ...
                    mask_path = '/root/autodl-tmp/attributeandonlyonestepcenterprompt/' + "blur" + str(index) + '.png'
                    cv2.imwrite(os.path.join(mask_path), mask_img)
...
def image_demo(predictor):
    ...
    for file in files:
        ...
        if(file != "refgta") and (file != "refcoco") and (file != "refcoco+"):
          for subFile in subFileDir:
            ...
            for i in range(len(data)):
                shutil.rmtree('/root/autodl-tmp/attributeandonlyonestepcenterprompt/')
                os.mkdir('/root/autodl-tmp/attributeandonlyonestepcenterprompt/')
                ...
                print(j, i, j / (i + 1))
                pre = j / (i + 1)
                path = "/root/autodl-tmp/result/attributeandonlyonestepcenterprompt/" + file + "_attributeandonlyonestepcenterprompt_" + annotationFileName
                fo = open(path, "a")
                fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

            attribute_result = open("/root/autodl-tmp/result/attributeandonlyonestepcenterprompt/attributeandonlyonestepcenterprompt.csv", "a")
