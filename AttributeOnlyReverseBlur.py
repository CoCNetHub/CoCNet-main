...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeonlyreversemask', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    mask_path = '/root/autodl-tmp/attributeonlyreversemask/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeonlyreversemask/' + "yuanben" + img_file[int(img_file.rfind('/')) + 1:]
...
class Predictor(object):
    ...
    def attribute(self, objects, img_file, subject='', subject1_and_subject2=0, subject1='', subject2=''):
        ...
        for index in range(len(objects)):
            ...
            mask_path = '/root/autodl-tmp/attributeonlyreversemask/' + "blur" + str(index) + '.png'
            cv2.imwrite(os.path.join(mask_path), mask_img)
            cropList.append({"bbox": bbox, "index": index, "cls_name": clsNameItem,
                             "mask_path": mask_path})
    ...
    for file in files:
        ...
        for subFile in subFileDir:
            ...
            for i in range(len(data)):
                shutil.rmtree('/root/autodl-tmp/attributeonlyreversemask/')
                os.mkdir('/root/autodl-tmp/attributeonlyreversemask/')
                ...
                if (len(cropList) > 1):
                    ...
                    similarity_list = np.sum([mask_similarity_list], axis=0).tolist()
                    max = 0
                ...
                print(j, i, j / (i + 1))
                pre = j / (i + 1)
                path = "/root/autodl-tmp/result/attributeonlyreversemask/" + file + "_attribute_" + annotationFileName
                fo = open(path, "a")
                fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

            attribute_result = open("/root/autodl-tmp/result/attributeonlyreversemask/attributeonlyreversemask.csv", "a")
