...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeandspatialproposal', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    mask_path = '/root/autodl-tmp/attributeandspatialproposal/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeandspatialproposal/' + "yuanben" + img_file[
...
class Predictor(object):
    ...
    def attribute(self, objects, img_file, subject=''):
        cropList = []
        ...
        for index in range(len(objects)):
            ...
            cropList.append({"bbox": bbox, "index": index, "crop_path": crop, "mask_path": mask_img, "blur_path": blur_all_img})
        return cropList

        return cropList

    def position(self, objects, img_file):
        positionList = []
        for index in range(len(objects)):
            bbox = objects[index]["bbox"]
            if int(bbox[0] < 0):
            ...
            if int(bbox[3] < 0):
                bbox[3] = 0
            positionList.append({"bbox": bbox, "index": index})
        return positionList
        ...
        if (file == "refgta"):
          for subFile in subFileDir:
            ...
            for i in range(len(data)):
                shutil.rmtree('/root/autodl-tmp/attributeandspatialproposal/')
                os.mkdir('/root/autodl-tmp/attributeandspatialproposal/')
                annotation_list = data.iloc[i]
                img_file_item, x1, y1, x2, y2, queries, objects = annotation_list
                annot = [x1, y1, x2, y2]
                objectList = [box.split(', ') for box in objects.strip('[]').split('], [')]
                img_file = "/root/autodl-tmp/refgta-val-test/" + img_file_item
                objects = predictor.visual(objectList)
                # 位置模块
                # 位置词
                ...
                for queryitem in queries_list:
                    ...
                    if (queryitem in preposition) and (queryitem in position_text):
                        position_text.remove(queryitem)
                cropList = predictor.attribute(objects, img_file)
                ...
                if (len(cropList) > 1):
                    ...
                    mask_images = preprocess(cropList[0]["mask_path"]).unsqueeze(0).to(device)
                    for index in range(1, len(cropList)):
                        mask_image = preprocess(cropList[index]["mask_path"]).unsqueeze(0).to(device)
                ...
                xflag = 0
                yflag = 0
                if (position_flag == 1) and (len(cropList) > 1):
                    positionList = predictor.position(objects, img_file)
                ...
                print(j, i, j / (i + 1))
                pre = j / (i + 1)
                path = "/root/autodl-tmp/result/attributeandspatialproposal/" + file + "_attributeandspatialproposal_" + annotationFileName
                fo = open(path, "a")
                fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

            attribute_result = open(
                "/root/autodl-tmp/result/attributeandspatialproposal/attributeandspatialproposal.csv", "a")
...
def read_annotations(trn_file):
    ...
    trn_df = trn_data[['img_id', 'x1', 'y1', 'x2', 'y2', 'query', 'proposals']]
    return trn_df
