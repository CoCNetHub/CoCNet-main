...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    colorRGB = [[255,0,0],[128,0,128],[0,0,255],[255,255,255],[0,128,0],[255,255,0],[165,42,42],[128,128,128],[0,0,0],
                [255,20,147],[0,191,255],[0,255,255],[255,215,0],[255,165,0],[255,0,255],[127,255,170],[128,128,0],[255,192,203]]
    ...
    i=-1
    zeros = np.ones((image.shape), dtype=np.uint8)
    for index in range(len(bboxes)):
        ...
        if(i < len(colorName)):
            ...
            shape = 'rectangle'
            zeros_mask = cv2.rectangle(zeros, (int(bboxes[index][0]), int(bboxes[index][1])),(int(bboxes[index][2]), int(bboxes[index][3])),color=(colorRGB[colorIndex][2], colorRGB[colorIndex][1], colorRGB[colorIndex][0]), thickness=-1)
        elif(i < 2*len(colorName)):
            colorIndex = colorIndexItem-len(colorName)
            colorList[colorIndex] = {'colorName': colorName[colorIndex], 'colorRGB': colorRGB[colorIndex]}
            shape = 'circle'
            zeros_mask = cv2.circle(zeros, (
            int((bboxes[index][0] + bboxes[index][1]) / 2), int((bboxes[index][1] + bboxes[index][1]) / 3)),(int(bboxes[index][2] - bboxes[index][0]), int(bboxes[index][3] - bboxes[index][1])),0, 0, 360,(colorRGB[colorIndex][2], colorRGB[colorIndex][1], colorRGB[colorIndex][0]), 3)
        else:
            ...
            cv2.drawContours(image, [triangle_cnt], 0, (colorRGB[colorIndex][2], colorRGB[colorIndex][1], colorRGB[colorIndex][0]), -1)
        objectList[index] = {'colorName':  colorName[colorIndex], 'colorRGB': colorRGB[colorIndex], 'boxArea': bboxes[i], 'shape': shape}
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attribute', img_name), mask_img)
...
def relation(cropList, img_file):
        ...
        # cv2.addWeighted 将原始图片 与 mask 融合
        mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
        mask_path = '/root/autodl-tmp/attribute/' + img_file[int(img_file.rfind('/')) + 1:]
        yuanben_path = '/root/autodl-tmp/attribute/' + "yuanben" + img_file[int(img_file.rfind('/')) + 1:]
...
class Predictor(object):
    ...
    def visual(self, output, img_info, cls_conf=0.5):
        ...
        for i in range(len(bboxes)):
            ...
            if (max_score < score):
                max_score = score
                max_object = [{"bbox": bboxes[i], "cls": cls[i], "cls_name": self.cls_names[int(cls[i])], "cls_conf": score}]
            if score < cls_conf:
                continue
            objectName.append(self.cls_names[int(cls[i])])
            objects.append({"bbox": bboxes[i],"cls": cls[i],"cls_name":self.cls_names[int(cls[i])],"cls_conf":score})
        objectName = list(set(objectName))
        return objects, objectName, max_object

    def attribute(self, objects, img_file, subject='', subject1_and_subject2 = 0, subject1 = '', subject2 = ''):
        cropList = []
        ...
        for index in range(len(objects)):
            ...
            crop_path = '/root/autodl-tmp/attribute/' + str(index) + '.png'
            ...
            mask_path = '/root/autodl-tmp/attribute/' + "blur" + str(index) + '.png'
            cv2.imwrite(os.path.join(mask_path), mask_img)
    ...
    for file in files:
        ...
        for subFile in subFileDir:
            ...
            for i in range(len(data)):
                shutil.rmtree('/root/autodl-tmp/attribute/')
                os.mkdir('/root/autodl-tmp/attribute/')
                ...
                print(j, i, j / (i + 1))
                pre = j / (i + 1)
                path = "/root/autodl-tmp/result/attribute/" + file + "_attribute_11_16_" + annotationFileName
                fo = open(path, "a")
                fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

            attribute_result = open("/root/autodl-tmp/result/attribute/attribute_11_16.csv", "a")
