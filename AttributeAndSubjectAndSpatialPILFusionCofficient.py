...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeandsubjectandspatialpilfusionConfficient', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    reverse_blur_path = '/root/autodl-tmp/attributeandsubjectandspatialpilfusionConfficient/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeandsubjectandspatialpilfusionConfficient/' + "yuanben" + img_file[int(img_file.rfind('/')) + 1:]
...
def image_demo(predictor):
    ...
    FusionCoefficientList = [0.3]
    for clip_model_list_item in clip_model_list:
        for FusionCoefficient in FusionCoefficientList:
            ...
            for file in files:
                ...
                for subFile in subFileDir:
                    ...
                    for i in range(len(data)):
                        shutil.rmtree('/root/autodl-tmp/attributeandsubjectandspatialpilfusionConfficient/')
                        os.mkdir('/root/autodl-tmp/attributeandsubjectandspatialpilfusionConfficient/')
                        ...
                        if (position_flag == 1):
                            ...
                            if (len(positionList) > 1):
                                ...
                                if (postionflag == 1):
                                    ...
                                    max_value = 0
                                    if (attribute_flag == 1):
                                        position_posibility_list = [i * FusionCoefficient for i in position_posibility_list]
                                        similarity_list = [i * (1-FusionCoefficient) for i in similarity_list]
                        ...
                        path = "/root/autodl-tmp/result/attributeandsubjectandspatialpilfusionConfficient/" + file + "_attributeandsubjectandspatialpilfusionConfficient_adjust_" + clip_model_list_item_str + '_' + str(FusionCoefficient) + annotationFileName
                        fo = open(path, "a")
                        fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

                    attribute_result_path = "/root/autodl-tmp/result/attributeandsubjectandspatialpilfusionConfficient/" + "attributeandsubjectandspatialpilfusionConfficient_adjust1_" + clip_model_list_item_str + '_' + str(FusionCoefficient) + ".csv"
