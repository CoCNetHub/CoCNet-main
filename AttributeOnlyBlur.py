...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeonlyblur', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    mask_path = '/root/autodl-tmp/attributeonlyblur/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeonlyblur/' + "yuanben" + img_file[int(img_file.rfind('/')) + 1:]
...
class Predictor(object):
    ...
    def attribute(self, objects, img_file, subject='', subject1_and_subject2=0, subject1='', subject2=''):
        ...
        for index in range(len(objects)):
            ...
            zheng_mask_path = '/root/autodl-tmp/masks2/' + "zheng_mask_path" + str(index) + '.png'
            zheng_mask_all_img = cv2.imread(img_file)
            zheng_mask_all_img = cv2.blur(zheng_mask_all_img, (40, 40))
            zheng_mask_all_img[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])] = crop
            cv2.imwrite(os.path.join(zheng_mask_path), zheng_mask_all_img)
            cropList.append({"bbox": bbox, "index": index, "cls_name": clsNameItem, "zheng_mask_path": zheng_mask_path})
    ...
    for file in files:
        ...
        for subFile in subFileDir:
            ...
            for i in range(len(data)):
                shutil.rmtree('/root/autodl-tmp/attributeonlyblur/')
                os.mkdir('/root/autodl-tmp/attributeonlyblur/')
                ...
                # 进入属性模块的条件
                if (len(cropList) > 1):
                    zheng_images = preprocess(Image.open(cropList[0]["zheng_mask_path"])).unsqueeze(0).to(device)
                    for index in range(1, len(cropList)):
                        zheng_mask_path = cropList[index]["zheng_mask_path"]
                        zheng_image = preprocess(Image.open(zheng_mask_path)).unsqueeze(0).to(device)
                    ...
                    with torch.no_grad():
                        ...
                        for index in range(len(zheng_mask_similarities)):
                            zheng_mask_similarity_list.append(zheng_mask_similarities[index][0])
                    similarity_list = np.sum([zheng_mask_similarity_list], axis=0).tolist()
                    max = 0
                ...
                print(j, i, j / (i + 1))
                pre = j / (i + 1)
                path = "/root/autodl-tmp/result/attributeonlyblur/" + file + "_attribute_" + annotationFileName
                fo = open(path, "a")
                fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

            attribute_result = open("/root/autodl-tmp/result/attributeonlyblur/attributeonlyblur.csv", "a")
