...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeandsmallsubjectandspatial', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    reverse_blur_path = '/root/autodl-tmp/attributeandsmallsubjectandspatial/' + img_file[
                                                                                         int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeandsmallsubjectandspatial/' + "yuanben" + img_file[
                                                                                                int(img_file.rfind(
                                                                                                    '/')) + 1:]
            ...
            for file in files:
                ...
                for subFile in subFileDir:
                    ...
                    for i in range(len(data)):
                        shutil.rmtree('/root/autodl-tmp/attributeandsmallsubjectandspatial/')
                        os.mkdir('/root/autodl-tmp/attributeandsmallsubjectandspatial/')
                        ...
                        path = "/root/autodl-tmp/result/attributeandsmallsubjectandspatial/" + file + "_attributeandsmallsubjectandspatial_adjust_" + clip_model_list_item_str + '_' + str(
                            CoefficientItem) + '_' + annotationFileName
                        fo = open(path, "a")
                        fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

                    attribute_result_path = "/root/autodl-tmp/result/attributeandsmallsubjectandspatial/" + "attributeandsmallsubjectandspatial_adjust1_" + clip_model_list_item_str + '_' + str(
                        CoefficientItem) + ".csv"
