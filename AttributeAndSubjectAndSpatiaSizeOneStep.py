...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeandsubjectandspatialsizeonestep', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    mask_path = '/root/autodl-tmp/attributeandsubjectandspatialsizeonestep/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeandsubjectandspatialsizeonestep/' + "yuanben" + img_file[
...
class Predictor(object):
    ...
    def visual(self, output, img_info, cls_conf=0.5):
        ...
        area = img_info["height"] * img_info["width"]
        if output is None:
            return img
        output = output.cpu()
        bboxes = output[:, 0:4]
        ...
        for i in range(len(bboxes)):
            score = scores[i]
            bboxes_index_width = bboxes[i][2] - bboxes[i][0]
            bboxes_index_height = bboxes[i][3] - bboxes[i][1]
            bboxes_area = bboxes_index_width * bboxes_index_height
            if (max_score < score):
                max_score = score
                max_object = [
            ...
            if (bboxes_area > (area * 0.05)):
                ...
                continue
            if(len(objectName)==0):
                objectName = [max_object["cls_name"]]
                objects = max_object
        ...
        for index in range(len(objects)):
            ...
            if (subject != ''):
                if (clsNameItem == subject):
                    ...
                    crop_path = '/root/autodl-tmp/attributeandsubjectandspatialsizeonestep/' + str(index) + '.png'
                    ...
                    mask_path = '/root/autodl-tmp/attributeandsubjectandspatialsizeonestep/' + "blur" + str(index) + '.png'
                    cv2.imwrite(os.path.join(mask_path), mask_img)
    ...
    for file in files:
        ...
        if(file=="refcoco+"):
            for subFile in subFileDir:
                ...
                globalPromptPath = subFileSecondDirPath + "/onestep_global_prompt/"
                ...
                centerPromptPath = subFileSecondDirPath + "/onestep_center_prompt/"
                ...
                for i in range(len(data)):
                    shutil.rmtree('/root/autodl-tmp/attributeandsubjectandspatialsizeonestep/')
                    os.mkdir('/root/autodl-tmp/attributeandsubjectandspatialsizeonestep/')
                    ...
                    if (len(cropList) > 1):
                        ...
                        elif (len(objects) == 0) or (len(cropList) == 0):
                            max_predict = [0, 0, 0, 0]
                        else:
                    ...
                    print(j, i, j / (i + 1))
                    pre = j / (i + 1)
                    path = "/root/autodl-tmp/result/attributeandsubjectandspatialsizeonestep/" + file + "_attributeandsubjectandspatialsizeonestep_" + annotationFileName
                    fo = open(path, "a")
                    fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

                attribute_result = open(
                    "/root/autodl-tmp/result/attributeandsubjectandspatialsizeonestep/attributeandsubjectandspatialsizeonestep.csv",
                    "a")
