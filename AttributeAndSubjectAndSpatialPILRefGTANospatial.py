...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeandsubjectandspatialpilrefgtanospatial3', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    reverse_blur_path = '/root/autodl-tmp/attributeandsubjectandspatialpilrefgtanospatial3/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeandsubjectandspatialpilrefgtanospatial3/' + "yuanben" + img_file[int(img_file.rfind('/')) + 1:]
        ...
        for index in range(len(objects)):
            ...
            if (subject != ''):
                if (clsNameItem == subject):
                    ...
                    cropList.append({"bbox": bbox, "index": index, "crop_path": crop, "cls_name": clsNameItem, "blur_path": blur_all_img})
            else:
                # crop picture
                crop = img.crop((int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])))
                # # Reverse blur
...
def image_demo(predictor):
    ...
    # clip_model_list = ["RN50", "RN101", "RN50x4", "RN50x64", "ViT-B/16", "ViT-L/14", "ViT-L/14@336px"]
    # clip_model_list = ["ViT-L/14", "ViT-L/14@336px"]
    ...
    for clip_model_list_item in clip_model_list:
        ...
        for file in files:
            ...
            for subFile in subFileDir:
                if(subFile!="testdt") and (subFile!="valdt"):
                    continue
                ...
                for i in range(len(data)):
                    shutil.rmtree('/root/autodl-tmp/attributeandsubjectandspatialpilrefgtanospatial3/')
                    os.mkdir('/root/autodl-tmp/attributeandsubjectandspatialpilrefgtanospatial3/')
                    ...
                    if (len(cropList) > 1):
                        ...
                        with torch.no_grad():
                            ...
                            for index in range(len(zheng_mask_similarities)):
                                zheng_mask_similarity_list.append(zheng_mask_similarities[index][0] * 0.5)
                        #
                        ...
                        similarity_list = np.sum([similarity_list, zheng_mask_similarity_list], axis=0).tolist()
                    ...
                    if (flag == "正确"):
                        ...
                        image = cv2.imread(img_file)
                        # 左上角和右下角
                        # (zeros, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), color=(0, 0, 255), thickness = -1)
                        visualize_image = cv2.rectangle(image, (int(max_predict[0]), int(max_predict[1])),(int(max_predict[2]), int(max_predict[3])),color=(0, 255, 255), thickness=2)
                        visualize_image = cv2.rectangle(visualize_image, (int(annot[0]), int(annot[1])),(int(annot[2]), int(annot[3])), color=(0, 0, 255), thickness=2)
                        # 图片显示
                        visualize_image_path = "/root/autodl-tmp/refgta/" + flag + str(queries) + str(i)+".png"
                        # 图片保存
                        cv2.imwrite(visualize_image_path, visualize_image)
                    print(j, i, j / (i + 1))
                    pre = j / (i + 1)
                    clip_model_list_item_str = clip_model_list_item.replace('/', '-')
                    path = "/root/autodl-tmp/result/attributeandsubjectandspatialpilrefgtanospatial3/" + file + "_attributeandsubjectandspatialpilrefgtanospatial3_" + clip_model_list_item_str + '' + annotationFileName
                    fo = open(path, "a")
                    fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

                attribute_result_path = "/root/autodl-tmp/result/attributeandsubjectandspatialpilrefgtanospatial3/"+"attributeandsubjectandspatialpilrefgtanospatial3_"+clip_model_list_item_str+".csv"
