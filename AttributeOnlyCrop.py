...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeonlycrop', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    mask_path = '/root/autodl-tmp/attributeonlycrop/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeonlycrop/' + "yuanben" + img_file[int(img_file.rfind('/')) + 1:]
...
class Predictor(object):
    ...
    def attribute(self, objects, img_file):
        cropList = []
        ...
        for index in range(len(objects)):
            ...
            crop_path = '/root/autodl-tmp/attributeonlycrop/' + str(index) + '.png'
    ...
    for file in files:
        ...
        for subFile in subFileDir:
            ...
            for i in range(len(data)):
                shutil.rmtree('/root/autodl-tmp/attributeonlycrop/')
                os.mkdir('/root/autodl-tmp/attributeonlycrop/')
                ...
                print(j, i, j / (i + 1))
                pre = j / (i + 1)
                path = "/root/autodl-tmp/result/attributeonlycrop/" + file + "_attribute_11_16_" + annotationFileName
                fo = open(path, "a")
                fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

            attribute_result = open("/root/autodl-tmp/result/attributeonlycrop/attributeonlycrop_11_16_.csv", "a")
