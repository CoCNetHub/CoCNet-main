...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeandsubjectandspatialTwo', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    reverse_blur_path = '/root/autodl-tmp/attributeandsubjectandspatialTwo/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeandsubjectandspatialTwo/' + "yuanben" + img_file[int(img_file.rfind('/')) + 1:]
        ...
        for file in files:
            ...
            for subFile in subFileDir:
                ...
                for i in range(len(data)):
                    shutil.rmtree('/root/autodl-tmp/attributeandsubjectandspatialTwo/')
                    os.mkdir('/root/autodl-tmp/attributeandsubjectandspatialTwo/')
                    ...
                    path = "/root/autodl-tmp/result/attributeandsubjectandspatialTwo/" + file + "_attributeandsubjectandspatialTwo_adjust_" + clip_model_list_item_str + '' + annotationFileName
                    fo = open(path, "a")
                    fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

                attribute_result_path = "/root/autodl-tmp/result/attributeandsubjectandspatialTwo/"+"attributeandsubjectandspatialTwo_adjust1_"+clip_model_list_item_str+".csv"
