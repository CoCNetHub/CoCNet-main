...
def plot_bbx(img_info, bboxes, scores, cls, cls_conf, cls_names):
    ...
    cv2.imwrite(os.path.join('/root/autodl-tmp/attributeandsubjectandspatialgt', img_name), mask_img)
...
def relation(cropList, img_file):
    ...
    # cv2.addWeighted 将原始图片 与 mask 融合
    mask_img = cv2.addWeighted(image, alpha, masks, beta, gamma)
    reverse_blur_path = '/root/autodl-tmp/attributeandsubjectandspatialgt/' + img_file[int(img_file.rfind('/')) + 1:]
    yuanben_path = '/root/autodl-tmp/attributeandsubjectandspatialgt/' + "yuanben" + img_file[int(img_file.rfind('/')) + 1:]
    ...
    for clip_model_list_item in clip_model_list:
        ...
        for file in files:
            ...
            for subFile in subFileDir:
                if(subFileDir=="refgta"):
                    continue
                subFileSecondDirPath = subFileDirPath + "/" + subFile

                annotationFilePath = subFileSecondDirPath + "/annotationGT/"
                ...
                for i in range(len(data)):
                    shutil.rmtree('/root/autodl-tmp/attributeandsubjectandspatialgt/')
                    os.mkdir('/root/autodl-tmp/attributeandsubjectandspatialgt/')
                    annotation_list = data.iloc[i]
                    img_file_item, x1, y1, x2, y2, queries, proposal, proposal_category = annotation_list
                    annot = [x1, y1, x2, y2]
                    ...
                    if (len(cropList) > 1):
                        ...
                        similarity_list = np.sum([similarity_list, mask_similarity_list, zheng_mask_similarity_list], axis=0).tolist()
                        ...
                        if (len(positionList) > 1):
                            ...
                            if (queries.find('next to') != -1) and (queries.find('middle') == -1) and (
                                ...
                                for index in range(1, len(objectList)):
                                    crop_image = preprocess(location_objects[index]["crop_path"]).unsqueeze(0).to(device)
                                    crop_images = torch.cat([crop_images, crop_image], dim=0)
                            ...
                            if (queries.find('between') != -1) and (queries.find('and') != -1) and (
                                ...
                                if (location_query.find('and') != -1):
                                    ...
                                    postionflag = 1
                                    location_objects = predictor.attribute(objects, img_file, '')
                                    crop_images = preprocess(location_objects[0]["crop_path"]).unsqueeze(0).to(device)
                                    for index in range(1, len(objectList)):
                                        crop_image = preprocess(location_objects[index]["crop_path"]).unsqueeze(0).to(device)
                                        crop_images = torch.cat([crop_images, crop_image], dim=0)
                    ...
                    path = "/root/autodl-tmp/result/attributeandsubjectandspatialgt/" + file + "_attributeandsubjectandspatialgt_" + clip_model_list_item_str + '' + annotationFileName
                    fo = open(path, "a")
                    fo.write(str(queries) + ',' + str(flag) + ',' + str(pre) + "\n")

                attribute_result_path = "/root/autodl-tmp/result/attributeandsubjectandspatialgt/"+"attributeandsubjectandspatialgt_"+clip_model_list_item_str+".csv"
...
def read_annotations(trn_file):
    ...
    if sample[0] == '[':
        trn_data['query'] = trn_data['query'].apply(
            lambda x: ast.literal_eval(x))
        trn_data['proposals'] = trn_data['query'].apply(
            lambda x: ast.literal_eval(x))
        trn_data['proposals_category'] = trn_data['proposals_category'].apply(
            lambda x: ast.literal_eval(x))
    ...
    trn_df = trn_data[['img_id', 'x1', 'y1', 'x2', 'y2', 'query', 'proposals', 'proposals_category']]
    return trn_df
